...
d=[67,53];
e=[76,30];
% hou=Plan_path2(5:end,:);
% qian=Plan_path2(1,:);
% hangdao=[25,76;28,35];
% % 
Plan_path2=[1,100;a;b;c;d;e;100,1];
...
plot(xTarget+.5,yTarget+.5,'bo');
text(xTarget+1,yTarget+1.5,'T','fontsize',12')

% h1=plot( x+1, y+1,'m','linewidth',2);
h2=plot( Plan_path2(:,1)+.5, Plan_path2(:,2)+.5,'b','linewidth',2);
% legend([h1,h2],'初步规划路径','二次优化路径','location','northeast');
%
% set(gca,'xtick',1:1:MAX_X+1,'ytick',1:1:MAX_Y+1,'GridLineStyle','-',...
%     'xGrid','on','yGrid','on')
aaa=linspace(121.7260,122.0380,4);
...
L_obst2=0.138;%0.016;% 设置移动障碍物的速度 0.1s内运动 L_obst m  速度为10*L_obst m/s
...
yuce=10;
Kinematic=[7.7175,toRadian(0.3244*3*yuce),0.2,toRadian(0.02*15*yuce),0.01,toRadian(0.5)];%5秒转25度，根据模型计算，给予模型三向适当力矩，求得船舶在航行时的转艏加速度
...
%%  评价系数
evalParam=[0.1,0.2,0.3,0.5,yuce];%最后一个参数看预测轨迹提前多少秒
path_node=Plan_path2;
Result_x=DWA_ct_dong4(Obs_Closed,Obst_d_d_line,Obst_d_d_line1,Obst_d_d_line2,Obs_d_j,Area_MAX,Goal,Plan_path2,path_node,Start,angle_node,Kinematic,evalParam);
