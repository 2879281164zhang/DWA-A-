function lineOPEN = Line_OPEN_ST(Optimal_path_ts, CLOSED, Num_obs, Num_Opt)
    ...
    while (n > 1)
        ...
        % 检查父节点和子节点是否一致
        if isequal(parent_node, parentP_node)
            % 如果一致，将下一个子节点作为父节点
            if index < Num_Opt
                parentP_node = [Optimal_path(index+1, 3), Optimal_path(index+1, 4)];
            else
                parentP_node = parentPP_node;
            end
            Optimal_path(v, 3) = parentP_node(1);
            Optimal_path(v, 4) = parentP_node(2);
        end
        ...
        if angle == 0 %%%夹角为0，说明在一条直线上，修改父节点
            ...
            x1 = parent_node(1, 1);
            y1 = parent_node(1, 2);
            x3 = parentPP_node(1, 1);
            y3 = parentPP_node(1, 2);
            k = (y3 - y1) / (x3 - x1);
            b = y1 - k * x1; %%两点之间直线方程 y=k*x+b
            x_min = min(x1, x3);
            x_max = max(x1, x3);
            y_min = min(y1, y3);
            y_max = max(y1, y3);
            ...
            for j = 1:1:Num_obs
                x_obn = CLOSED(j, 1);
                y_obn = CLOSED(j, 2); %%判断障碍点是否在行驶区域内
