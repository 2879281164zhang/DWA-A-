function Tg=Target_node(dang_node,path,Obs_dong,xTarget,yTarget,goal,dis_x_du)
   ...
   path_dis=[];
   distan_d_g = distance(dang_node(1,1),dang_node(1,2),goal(1,1),goal(1,2));%当前船舶与障碍物的距离
   ...
   for i=1:1:num_Obs_dong  %计算目标点到动态障碍物的距离
    ...
    %改船舶到局部目标点（船舶离局部目标点多远的时候，局部目标点更换为下一个目标点）
    if n_index<num_path
       if distan_d_g<5 || ( min_dis<5&&dis_d_minObs<5) || dis_x_du<2
