...
cfg.inputMaskFile        = '3_chart_masks_clean.mat';
cfg.autoFixMaskPolarity  = true;   % 自动修正黑白反置问题
...
% 可选：使用新航道掩膜文件覆盖地图栅格（航道中间开口版本）
if ~isempty(cfg.inputMaskFile) && exist(cfg.inputMaskFile, 'file') == 2
    [G2, mapInfo2] = loadNavigableMaskFromMatLocal(cfg.inputMaskFile, mapInfo, G);
    G = G2;
    mapInfo = mapInfo2;
    fprintf('Using navigable mask from: %s\\n', cfg.inputMaskFile);
end

latRange = mapInfo.latRange;
lonRange = mapInfo.lonRange;

% 自动极性修正：用原始路径点在可航区域的覆盖率判断黑白是否反了
if cfg.autoFixMaskPolarity
    r1 = pathFreeRatioLocal(pathLonRaw, pathLatRaw, ~G, latRange, lonRange);
    r2 = pathFreeRatioLocal(pathLonRaw, pathLatRaw, G,  latRange, lonRange);
    if r2 > r1 + 0.05
        G = ~G;
        fprintf('Mask polarity auto-corrected (black/white was reversed).\\n');
    end
end
...
function [Gout, mapInfoOut] = loadNavigableMaskFromMatLocal(maskFile, mapInfoIn, Gfallback)
% 从新掩膜mat中提取可航栅格，转换成障碍栅格G（障碍=true）
T = load(maskFile);
fns = fieldnames(T);

Gout = logical(Gfallback);
mapInfoOut = mapInfoIn;

% 1) 优先按变量名查找
nameOrder = {'navigableMask','navigable_mask','navigable','mask','BW','freeMask','G'};
found = false;
raw = [];
for i = 1:numel(nameOrder)
    nm = nameOrder{i};
    if isfield(T, nm) && isnumeric(T.(nm)) && ismatrix(T.(nm))
        raw = T.(nm);
        found = true;
        break;
    end
...
if found
    raw = real(raw);
    raw = raw ~= 0;
    % 对“navigable/free”语义变量，true通常表示可航；G要求true表示障碍
    if exist('nm','var') && contains(lower(nm), 'navig')
        Gout = ~raw;
    elseif exist('nm','var') && contains(lower(nm), 'free')
        Gout = ~raw;
    elseif exist('nm','var') && strcmpi(nm, 'G')
        Gout = raw;
    else
        % 默认按“1=可航”解释
        Gout = ~raw;
...
% 地理范围信息（若掩膜文件中提供）
if isfield(T, 'mapInfo') && isstruct(T.mapInfo)
    m2 = T.mapInfo;
    if isfield(m2, 'latRange'), mapInfoOut.latRange = m2.latRange; end
    if isfield(m2, 'lonRange'), mapInfoOut.lonRange = m2.lonRange; end
end
if isfield(T, 'latRange'), mapInfoOut.latRange = T.latRange; end
if isfield(T, 'lonRange'), mapInfoOut.lonRange = T.lonRange; end
end
...
function ratio = pathFreeRatioLocal(lon, lat, freeMask, latRange, lonRange)
lon = real(lon(:)); lat = real(lat(:));
valid = isfinite(lon) & isfinite(lat);
lon = lon(valid); lat = lat(valid);
if isempty(lon)
    ratio = 0;
...
cnt = 0;
for i = 1:numel(lon)
    [r, c] = geoToRCIndexLocal(lon(i), lat(i), size(freeMask), latRange, lonRange);
    if freeMask(r, c)
        cnt = cnt + 1;
    end
end
