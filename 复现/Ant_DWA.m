...
grid on; axis square;                                   %%%  在画图的时候添加网格线
...
% Plan_path2([1,end],:)=Plan_path2([end,1],:);
% Plan_path2([2,3],:)=Plan_path2([3,2],:);
 n=size(Optimal_path_try,1);
   for i=1:1:n
       Optimal_path(i,1)=Optimal_path_try(n,3);
       Optimal_path(i,2)=Optimal_path_try(n,4);
...
if(xval1>=Obst_xS1&&yval1>Obst_yS1)
    angle11 =  atand(dy1/dx1);
elseif(xval1<Obst_xS1&&yval1>=Obst_yS1)
    angle11 = 180 + atand(dy1/dx1);
elseif(xval1<=Obst_xS1&&yval1<Obst_yS1)
    angle11 =  atand(dy1/dx1)+180;
elseif(xval1>Obst_xS1&&yval1<=Obst_yS1)
    angle11 =  360+atand(dy1/dx1);
else
    angle11 =  0;
end
angle11=-((360-angle11)/180)*pi;
angle_node=angle11;
...
L_obst2=0.06471;%0.016;% 设置移动障碍物的速度 0.1s内运动 L_obst m  速度为10*L_obst m/s
...
yuce=10;
Kinematic=[7.7175,toRadian(0.3244*10*yuce),0.1642,toRadian(0.0167*10*yuce),0.02,toRadian(0.1)];%5秒转25度，根据模型计算，给予模型三向适当力矩，求得船舶在航行时的转艏加速度
...
%%  评价系数            
evalParam=[alpha,beta,gama,yuce];%最后一个参数看预测轨迹提前多少秒
