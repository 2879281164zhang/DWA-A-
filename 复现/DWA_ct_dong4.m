...
function a = DWA_ct_dong4(Obs_Closed,Obs_dong,Obs_dong1,Obs_dong2,Obs_d_j,Area_MAX,Goal,Line_path,path_node,Start0,angle_node,Kinematic,evalParam);
global dist angle10 angle12 angle13 x
figure(5);
h=figure(5);
set(h,'position',[195 155 1165 500]);
...
for i=1:5000
    ...
    angle_node=angle10;
    dxx=0.5.*[5/8,3/4,5/8,-0.25,-0.25,5/8]';dyy=0.5.*[1/14,0,-1/14,-1/14,1/14,1/14]';
...
%     quiver(x(1)+0.5, x(2)+0.5, ArrowLength*cos(x(3)), ArrowLength*sin(x(3)), 'm'); % 绘制机器人当前位置的航向箭头
%     hold on;                                                     %启动图形保持功能，当前坐标轴和图形都将保持，从此绘制的图形都将添加在这个图形的基础上，并自动调整坐标轴的范围
    ...
    grid off; 
    %绘制窗口
    pgon = polyshape([x(1)-5 x(1)-5 x(1)+10 x(1)+10],[x(2)+5 x(2)-10 x(2)-10 x(2)+5]);
    plot(pgon,'FaceColor','none','FaceAlpha',0.1,'Linestyle','--','EdgeColor','r');
% drawnow
...
% 更新右边子图
  ...
  %% 设计规则评价函数，第一个船舶对遇局面
        ...
        if dist <=15
            if abs(180-(x(3)-angle10))<(2.5/180)*pi
                Rr=x(3)-angle10
            elseif abs(x(3)-angle12)<(67.5/180)*pi
                Rr=1/(x(3)-angle10)
            elseif abs(x(3)-angle13)>=(182.5/180)*pi & abs(x(3)-angle13)<=(292.5/180)*pi
                Rr=x(3)-angle10
            end
        end    
    dong_num=size(Obs_d_j,1);
...
drawnow
    ...
    movcount=movcount+1;
    mov(movcount) = getframe(gcf);%
v = VideoWriter('movie.avi');  % 如果你希望保存为 MP4，可写为 'movie.mp4', 'MPEG-4'
v.FrameRate = 10;              % 设置帧率（可根据你的仿真速度调整）
